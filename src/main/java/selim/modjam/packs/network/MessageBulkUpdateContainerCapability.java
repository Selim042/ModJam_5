...
public abstract class MessageBulkUpdateContainerCapability<HANDLER, DATA> implements IMessage {
	...
	/**
	 * The capability data instances for each slot, indexed by their index in
	 * the original {@link List<ItemStack>}.
	 */
	...
	public MessageBulkUpdateContainerCapability(final Capability<HANDLER> capability,
			@Nullable final EnumFacing facing, final int windowID, final NonNullList<ItemStack> items) {
		this.capability = capability;
		this.facing = facing;
	...
	public abstract static class Handler<HANDLER, DATA, MESSAGE extends MessageBulkUpdateContainerCapability<HANDLER, DATA>>
		...
		public final IMessage onMessage(MESSAGE message, MessageContext ctx) {
			...
			ModJamPacks.proxy.getThreadListener(ctx).addScheduledTask(() -> {
				final EntityPlayer player = ModJamPacks.proxy.getPlayer(ctx);
				...
				message.capabilityData.forEachEntry((index, data) -> {
					...
					final HANDLER originalHandler = originalStack.getCapability(message.capability,
							message.facing);
					if (originalHandler != null) {
						final ItemStack newStack = originalStack.copy();

						final HANDLER newHandler = newStack.getCapability(message.capability,
								message.facing);
		...
		/**
		 * Apply the capability data from the data instance to the capability
		 * handler instance.
		 *
		 * @param handler
